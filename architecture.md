...
4. **Save** — on submit, if `name` is non-empty after trimming (native `required` validation per FR-6, plus a `.trim()` check so a whitespace-only name doesn't pass browser validation but still fails intent), `EmployeeRow` disables the Save button and calls `updateEmployee(id, { name, department })` → `PUT /api/employees/:id` → `employeeStore.update()` persists to `employees.json` → response returns the updated record. `EmployeeDirectory` then calls `listEmployees()` again to refresh from the server (consistent with the existing `refresh()` pattern used after add/delete) and clears `editingId`. The Save button re-enables once the request settles, preventing duplicate submits from a double-click.
5. **Cancel** — `EmployeeRow` discards local form state and clears `editingId` in the parent; no network call is made (FR-5).
6. **Stale edit (404)** — if the employee was deleted by another action between load and save, the `PUT` 404s. `EmployeeRow` surfaces the error (reusing `EmployeeDirectory`'s existing `setError` pattern) and clears `editingId` rather than leaving a dangling edit form for a record that no longer exists.
7. **Persistence** — because step 4 writes through the existing JSON-file store, a page refresh reflects the saved values with no additional work.
...
- Server-side name validation on `PUT` remains absent (the store will happily persist an empty or whitespace-only name if the API is called directly, e.g. via curl). This is a known gap, reviewed and **accepted as out of scope**: `requirements.md` explicitly excludes backend changes, and client-side validation (including the trim check in Data Flow step 4) covers the UI-driven path.
- No optimistic-concurrency check exists: if the same employee is edited in two browser tabs at once, the second Save silently overwrites the first with no conflict warning. Reviewed and **accepted as low risk**: this app is single-local-admin, no-auth, local-use-only (per `CLAUDE.md`), so concurrent edits to the same row are unlikely; a version/`updatedAt` guard would be disproportionate for this story.
- No automated test suite or CI pipeline exists in this repo (`api/` and `ui/` `package.json` have no test scripts or framework configured). This is a pre-existing, app-wide gap rather than something introduced by this feature, so it is out of scope for this story but should be tracked separately before further production-bound changes accumulate without regression coverage.
